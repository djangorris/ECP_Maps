...
# FILTER CATEGORY
CO_ALL_ECP_BIND_DENTAL <- filter(CO_ALL_ECP_BIND,
                                                  ECP_Category == "Dental Providers")
# DISPLAY MAP AND ADD DATA POINTS
ggmap(co_map, extent = "device") +
  geom_density2d(data = CO_ALL_ECP_BIND_DENTAL,
                 aes(x = lon, y = lat),
                 size = 0.3) +
  stat_density2d(data = CO_ALL_ECP_BIND_DENTAL,
  ...
  geom_point(aes(lon, lat, color = "red"),
             shape = 21,
             data = CO_ALL_ECP_BIND_DENTAL,
             position = position_jitter(w = 0.002, h = 0.002)) +
  facet_wrap(~Carrier, ncol = 3) +
  xlab(" ") +
  ylab(NULL) +
  ggtitle('2018 Colorado Essential Dental Community Providers & Facilities') +
  ...
  theme(plot.margin = margin(5, 5, 5, 5),
        ...
        plot.caption = element_text(family = "Arial",
                                    ...
                                    color = "grey50",
                                    hjust = 0.5)) +
  ggsave(filename = "BINDED_ECPs/Plots/DENTAL.png", width = 9, height = 8, dpi = 1200)
