...
# FILTER CATEGORY
CO_ALL_ECP_BIND_TUBERCULOSIS <- filter(CO_ALL_ECP_BIND,
                                                  ECP_Category == "Tuberculosis Clinics")
# DISPLAY MAP AND ADD DATA POINTS
ggmap(co_map, extent = "device") +
  # geom_density2d(data = CO_ALL_ECP_BIND_TUBERCULOSIS,
  #                aes(x = lon, y = lat),
  #                size = 0.3) +
  # stat_density2d(data = CO_ALL_ECP_BIND_TUBERCULOSIS,
  ...
  geom_point(aes(lon, lat),
             ...
             fill = "green4",
             alpha = 0.5,
             data = CO_ALL_ECP_BIND_TUBERCULOSIS,
             position = position_jitter(w = 0.002, h = 0.002)) +
  facet_wrap(~Carrier, ncol = 4) +
  xlab(" ") +
  ylab(NULL) +
  ggtitle('2018 Colorado Essential Community Tuberculosis Clinics') +
  labs(caption = "\n\n  Graphic by Colorado Health Insurance Insider | @lukkyjay                                             Source: SERFF") +
  theme_ECP_maps +
  ggsave(filename = "BINDED_ECPs/Plots/TUBERCULOSIS.png", width = 9, height = 5, dpi = 1200)
