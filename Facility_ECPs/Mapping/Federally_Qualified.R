...
# FILTER CATEGORY
CO_all_medical_facility_carriers_FED_QUALIFIED <- filter(CO_all_medical_facility_carriers,
                                                              ECP_Category == "Federally Qualified Health Centers" |
                                                                ECP_Category == " Federally Qualified Health Centers")
CO_all_facility_carrier_count_FED_QUALIFIED <- filter(CO_all_facility_carrier_count,
                                                           ECP_Category == "Federally Qualified Health Centers" |
                                                             ECP_Category == " Federally Qualified Health Centers")
# DISPLAY MAP AND ADD DATA POINTS
ggmap(co_map, extent = "device") +
  geom_density2d(data = CO_all_medical_facility_carriers_FED_QUALIFIED,
                 aes(x = lon, y = lat),
                 size = 0.3) +
  stat_density2d(data = CO_all_medical_facility_carriers_FED_QUALIFIED,
  ...
  geom_point(aes(lon, lat, color = "red"),
             shape = 21,
             data = CO_all_medical_facility_carriers_FED_QUALIFIED,
             position = position_jitter(w = 0.002, h = 0.002)) +
  facet_wrap(~Carrier, ncol = 3) +
  xlab(" ") +
  ylab(NULL) +
  ggtitle("2018 Colorado Federally Qualified Health Centers (Facility ECPs)") +
  ...
  theme(plot.margin = margin(5, 5, 5, 5),
        ...
        plot.caption = element_text(family = "Arial",
                                    ...
                                    color = "grey50",
                                    hjust = 0.5)) +
  ggsave(filename = "Facility_ECPs/Plots/FED_QUALIFIED.png", width = 9, height = 8, dpi = 1200)
